...
export async function POST(request) {
  try {
    ...
    const instruction = `인테리어필름 시공 사진 ${files.length}장을 비교하라. 번호는 0부터 시작한다. 각 사진의 시공 부위(category: 싱크대/문·문틀/중문/붙박이장/신발장/현관문/기타), 세부 부위(sub_category), 전후(before/after/unknown), 현장 번호(site_key: 1,2,...), description, tags를 판별하라. referenceSite가 있는 사진은 이전 묶음의 현장 기준 사진이다. 시각적으로 같은 현장은 기준 사진과 동일한 site_key를 사용하고 다른 현장은 새로운 site_key를 사용하라. 다른 날짜라도 GPS와 타일·손잡이·배치 같은 구조가 일치하면 같은 현장일 수 있다. 같은 날이라도 다른 공간은 분리한다. 전후 상태를 같은 부위 사진끼리 비교하고 시각은 보조로만 사용한다. 단독 사진이나 불확실한 사진은 unknown. GPS 근접만으로 현장을 확정하지 말라. 사진 정보: ${JSON.stringify(metadata.map((m, index) => ({ index, takenAt: m.takenAt, latitude: m.latitude, longitude: m.longitude, referenceSite: m.referenceSite })))}. JSON만 반환: {"photos":[{"index":0,"site_key":"1","category":"싱크대","sub_category":"하부장","photo_type":"unknown","confidence":"low","description":"사진에 보이는 사실","tags":["특징"]}]}. 모든 index를 정확히 한 번씩 포함한다. confidence는 high/medium/low.`;
