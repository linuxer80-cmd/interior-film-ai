...
export default function Home() {
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      const historyMatches = (matchedPhotos || []).filter(
        (item) =>
          !item.photo_type ||
      ...
      setMessage(
        `✅ 분석 완료! 과거 시공사례 ${costs.length}건을 기준으로 견적을 계산했습니다.`
      );
    } catch (error) {
      console.error(error);
