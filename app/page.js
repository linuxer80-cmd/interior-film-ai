...
export default function Home() {
  ...
  async function prepareImage(file, maxSize = 1200, quality = 0.68) {
    if (!file) {
    ...
    try {
      ...
      try {
        ...
        console.log("createImageBitmap 사용 불가", error);
      }
      ...
      const blob = await new Promise((resolve, reject) => {
        canvas.toBlob(
          (result) => {
            if (!result) {
              reject(new Error("JPEG 이미지 변환에 실패했습니다."));
              return;
            }
            resolve(result);
          },
      ...
      throw new Error(
        "이 사진을 처리할 수 없습니다. 다른 사진을 선택하거나 카메라로 다시 촬영해주세요."
      );
    }
  }
  ...
  async function addImages(fileList) {
    ...
    setMessage(`사진을 준비하고 있습니다... 0/${selected.length}`);
    resetResults();
    ...
    try {
      ...
      for (let i = 0; i < selected.length; i += 1) {
        ...
        try {
          const prepared = await prepareImage(selected[i], 1200, 0.68);
          ...
          console.error(`사진 ${i + 1} 처리 실패`, error);
        }
      }
      ...
      if (additions.length < selected.length) {
        ...
        setMessage(`✅ 사진 ${additions.length}장을 준비했습니다.`);
      }
    } catch (error) {
      console.error(error);
      setMessage(
        `❌ 오류: ${error?.message || "사진을 불러올 수 없습니다."}`
      );
    } finally {
  ...
  function normalizeCategory(value) {
    ...
    if (text.includes("붙박이장") || text.includes("옷장")) {
      return "closet";
    }

    if (text.includes("신발장") || text.includes("현관장")) {
      return "shoe";
    }

    if (text.includes("화장대") || text.includes("서랍장")) {
      return "vanity";
    }
    ...
    if (text.includes("몰딩") || text.includes("걸레받이")) {
      return "molding";
    }

    if (text.includes("아트월") || text.includes("벽체")) {
      return "wall";
    }
  ...
  function getGroupKey(analysis) {
    return normalizeCategory(
      `${analysis?.category || ""} ${analysis?.sub_category || ""}`
    );
  }
  ...
  async function analyzeOnePhoto(imageItem, index, total) {
    setMessage(`AI 사진 분석 중... ${index + 1}/${total}`);
  ...
  async function findSimilarCases(group) {
    const analyses = group.photos.map((item) => item.analysis);
    ...
    const embeddingResult = await readJsonSafely(embeddingResponse);

    if (!embeddingResponse.ok || !embeddingResult?.embedding) {
      throw new Error(
    ...
    if (error) {
      throw new Error(`유사사례 검색 오류: ${error.message}`);
    }
  ...
  function calculateEstimate(cases) {
    ...
    const weightedAverage = weightedCostTotal / weightTotal;

    const min =
      Math.round((weightedAverage * 0.9) / 1000) * 1000;

    const max =
      Math.round((weightedAverage * 1.1) / 1000) * 1000;

    const average =
      Math.round(weightedAverage / 1000) * 1000;

    const topSimilarity = Math.max(
      ...cases.map((item) => Number(item.similarity || 0))
    );
    ...
    if (cases.length >= 5 && topSimilarity >= 0.85) {
      confidence = "높음";
    } else if (cases.length >= 2 && topSimilarity >= 0.75) {
      confidence = "보통";
    }
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      setMessage("같은 시공 부위의 사진을 묶고 있습니다...");
      ...
      for (const photo of analyzedPhotos) {
        ...
        if (!groupMap.has(key)) {
          groupMap.set(key, {
            key,
            category: photo.analysis?.category || "시공 부위",
            subCategory: photo.analysis?.sub_category || "",
            photos: [],
          });
        }
      ...
      for (let i = 0; i < baseGroups.length; i += 1) {
        ...
        setMessage(
          `유사 시공사례 검색 중... ${i + 1}/${baseGroups.length} · ${group.category}`
        );
        ...
        try {
          ...
          groupEstimate = calculateEstimate(cases);
        } catch (error) {
          console.error(error);
        }

        const casesWithImages = await Promise.all(
          cases.slice(0, 2).map(async (item) => ({
            ...item,
            beforeUrl: await getSignedImageUrl(item.before_path),
            afterUrl: await getSignedImageUrl(item.after_path),
          }))
        );
      ...
      setMessage(
        `❌ 오류: ${error?.message || "분석 중 오류가 발생했습니다."}`
      );
    } finally {
      setLoading(false);
    }
  ...
  async function uploadLeadPhotos() {
    ...
    for (let i = 0; i < images.length; i += 1) {
      ...
      if (error) {
        console.error(`상담 사진 ${i + 1} 저장 실패:`, error);
        continue;
      }
  ...
  async function handleLeadSubmit(event) {
    ...
    setLeadLoading(true);
    setLeadMessage("사진과 상담 신청을 접수하고 있습니다...");
    ...
    try {
      ...
      const estimateDetails = groups.map((group) => ({
        ...
        confidence: group.estimate?.confidence || "데이터 부족",
        similar_count: group.estimate?.count || 0,
      }));

      const description = groups
        .map((group, index) => {
          const descriptions = group.photos
            .map((photo) => photo.analysis?.description || "")
            .filter(Boolean)
            .join(" / ");
      ...
      const response = await fetch("/api/lead", {
        ...
        body: JSON.stringify({
          ...
          estimate_average: totalEstimate?.average ?? null,
          customer_photo_path: customerPhotoPaths[0] || null,
      ...
      if (!response.ok || !result?.success) {
        throw new Error(result?.error || "상담 신청 저장 오류");
      }
      ...
      setLeadMessage(
        `❌ 상담 신청 오류: ${error?.message || "다시 시도해주세요."}`
      );
    } finally {
  ...
  return (
    ...
    >
      ...
      >
        여러 시공 부위의 사진을 한 번에 올려주세요. AI가 같은
        부위의 사진을 묶어 각각 견적을 계산하고 마지막에 총
        예상견적을 보여드립니다.
      </p>
      ...
      <section style={sectionStyle}>
        ...
        <p style={{ color: "#6b7280", lineHeight: "1.6" }}>
          같은 부위를 여러 각도에서 촬영해도 됩니다. 다른 시공
          부위 사진도 함께 선택할 수 있습니다. 최대 10장까지
          가능합니다.
        </p>
        ...
        <button
          ...
          disabled={loading || imageLoading || !images.length}
          style={{
            width: "100%",
            marginTop: "18px",
            padding: "18px",
            ...
            cursor: "pointer",
            opacity:
              loading || imageLoading || !images.length ? 0.65 : 1,
          }}
        >
          {imageLoading
            ? "사진 준비 중..."
      ...
      {groups.length > 0 && (
        <section style={sectionStyle}>
          <h2 style={{ marginTop: 0 }}>AI 부위별 분석</h2>
            ...
            >
              ...
              {group.estimate ? (
                ...
                >
                  ...
                  <br />
                  유사 시공 {group.estimate.count}건 · 신뢰도{" "}
              ...
              {group.similarItems?.length > 0 && (
                <div style={{ marginTop: "15px" }}>
                  ...
                  {group.similarItems.map((item, caseIndex) => (
                    <div
                      key={item.work_item_id || caseIndex}
                    ...
                    >
                      ...
                      >
                        실제 시공금액{" "}
                        <strong>{formatWon(item.actual_cost)}원</strong>
                        {" · "}
                        유사도{" "}
                        {(
      ...
      {totalEstimate && (
        ...
        >
          ...
          >
            가중 평균 합계{" "}
            <strong>{formatWon(totalEstimate.average)}원</strong>
            <br />
            견적 계산 완료{" "}
            <strong>
          ...
          {totalEstimate.missingCount > 0 && (
            <p style={{ color: "#b45309", lineHeight: "1.6" }}>
              ⚠️ 데이터가 부족한 {totalEstimate.missingCount}개
              부위는 위 총액에 포함되지 않았습니다. 상담 시 함께
              확인합니다.
            </p>
          )}
          ...
          >
            실제 과거 시공금액을 기반으로 계산한 예상견적입니다.
            수량·크기·현장상태·자재 및 추가 작업에 따라 최종
            금액은 달라질 수 있습니다.
          </p>
        </section>
      )}
      ...
      {groups.length > 0 && (
        ...
        >
          ...
          >
            보내주신 사진 전체와 부위별 AI 견적을 담당자가 확인한
            뒤 안내해드립니다.
          </p>
          ...
          {leadComplete ? (
            ...
            <form onSubmit={handleLeadSubmit}>
              ...
              >
                ...
                상담을 위한 이름, 연락처, 시공지역, 고객 사진 및
                견적 정보 수집과 상담 연락에 동의합니다.
              </label>
